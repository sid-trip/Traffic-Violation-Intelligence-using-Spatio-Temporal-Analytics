...
def process_frame(img, conf_threshold=0.25):
    ...
    return restricted_polygon, detections

def evaluate_spatial_violations(detections, restricted_polygon):
    violations = []
    polygon = np.array(restricted_polygon, dtype=np.int32)
    for det in detections:
        x1, y1, x2, y2 = det["bbox"]
        bottom_center = (int((x1 + x2) / 2), int(y2))
        is_inside = cv2.pointPolygonTest(polygon, bottom_center, False)
        if is_inside >= 0:
            violations.append({
                "bbox": det["bbox"],
                "violation_type": "Restricted Zone Violation",
                "violation_confidence": det["confidence"]
            })
    return violations
...
st.title("Contextual Traffic Enforcement System")
st.markdown("Module: Spatial Overlap Inference and Violation Classification (MVP)")
st.markdown("---")
...
with st.sidebar:
    ...
    camera_id = st.selectbox(
        "Select Target Node", 
        ["CAM_001_INDIRANAGAR", "CAM_002_KORAMANGALA", "CAM_035_MGROAD"]
    )
    confidence_thresh = st.slider("Detection Confidence Threshold", 0.0, 1.0, 0.70)
    st.markdown("---")
    ...
    if uploaded_file is not None:
        ...
        selected_image.save(temp_input_path)
    else:
        if os.path.exists("test_image.jpg"):
            selected_image = Image.open("test_image.jpg")
            selected_image.save(temp_input_path)
...
if selected_image is not None:
    ...
    with col1:
        st.subheader("Source Image (Edge Node)")
        st.image(selected_image, use_container_width=True)
    if st.button("Execute Spatial Inference", type="primary"):
        with st.spinner("Executing spatial intersection evaluation and historical lookups..."):
            cv_img = cv2.imread(temp_input_path)
            restricted_polygon, detections = process_frame(cv_img, confidence_thresh)
            violations = evaluate_spatial_violations(detections, restricted_polygon)
            current_time_str = datetime.now().strftime("%H:%M")
            intelligence_data = get_historical_context(camera_id, current_time_str, violations)
            output_image_path = generate_evidence_image(
                cv_img,
                restricted_polygon,
                detections,
                intelligence_data["enriched_violations"],
                camera_id,
                current_time_str,
                intelligence_data
            )
            with col2:
                st.subheader("Annotated Output (Inference Result)")
                st.image(output_image_path, use_container_width=True)
        st.markdown("---")
        st.subheader("System Execution Logs")
        st.code(f"""
[INFO] Initializing inference on node {camera_id}
[INFO] Object detection completed. Found {len(detections)} target(s) exceeding confidence threshold {confidence_thresh}.
[INFO] Computing spatial overlap with configured node polygons...
[WARN] Spatial intersection evaluated. Detected {len(violations)} active violation(s).
[INFO] Querying historical context via BTP OpenCity datasets...
[INFO] Location matched to recurring hotspot parameters.
...
[INFO] Inference sequence completed successfully.
        """, language="bash")
        st.subheader("Inference Metadata Payload")
        st.json(intelligence_data)
else:
    st.write("Awaiting input selection. Please configure data ingestion parameters in the sidebar.")